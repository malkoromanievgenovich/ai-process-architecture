...
- **One door in, one door out:** the system meets the outside world through a single named gateway. Each external address — backend URL, API endpoint, third-party host — lives in exactly one place, and every caller imports it from there; no module hardcodes its own copy or opens a private channel. The wall is closed by default. A genuinely new need is met by widening that one door deliberately, not by punching a fresh hole wherever it is convenient. Scattered external references are leaks: many doors are many seams to miss, and every unclosed one is a gap — in security and in the boundary a human must be able to see and hold.
